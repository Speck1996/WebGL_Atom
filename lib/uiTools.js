...
var uiTools = {
	...
	setUpListeners: function(){
	      ...
	      window.addEventListener("keyup", keyFunctionSwitchCam , false);
				window.addEventListener("keyup", this.switchOrbit , false);
	...
	openNav: function() {
		var botHeight = document.getElementById("botPanel").style.height;
		if(	botHeight != 0){
			this.closeBotNav(event);
		}
  	document.getElementById("mySidepanel").style.width = "250px";
	},

	// it may happen that sometimes the closeSideNav is not triggered
	// when circumnavigating the panel
	closeSideNav: function(event){
...
/* Set the width of the sidebar to 0 (hide it) */
	fixedCloseSideNav: function(event) {
		// fix useful when firefox triggers the mouse leave when clicking on items
	...
	/* Set the width of the sidebar to 250px (show it) */
	openBotNav: function() {
		document.getElementById("botPanel").style.height = "220px";

	},

/* Set the width of the sidebar to 0 (hide it) */
	closeBotNav: function(event) {

			document.getElementById("botPanel").style.height = "0";
			document.activeElement.blur();
	},


	changeOrbitState: function(){
		orbitVisib = !orbitVisib;
	},

	switchOrbit: function(event){
		if (event.keyCode == 79){
			uiTools.changeOrbitState();
		}
	},

	setTrajConst: function(event){
		var slider = document.getElementById("myRange");
		trajConst = 1/slider.value * trajSliderCoeff;
	},

	resetCamModelParams: function(){
		setDefaultCam();
		orbitVisib = true;
		setCMesh();

		// resetting slider and traj constant
		var slider = document.getElementById("myRange");
		slider.value = 10;
		trajConst = 300;
