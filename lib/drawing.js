...
function main() {
  ...
  function animate(){
    ...
    if(lastUpdateTime){
      ...
      for(var i = 0; i < currentAtom.trajectories.length; i++){
         ...
         var deltaAlpha = (currTraj.constOrbit/Math.pow(currTraj.dist(), 2) * (currentTime - lastUpdateTime));

         // updating the current positions of the spheres in the given
         // trajectory
         currTraj.updateAlpha(deltaAlpha);
    ...
    for ( var i = 0; i < currentAtom.trajectories.length; i++){
      ...
      var nextPositions = currentAtom.trajectories[i].gibePos();
      ...
      for(var j = 0; j < nextPositions.length; j++){
        ...
        sphereWorldMatrix[k] = utils.MakeWorld( nextPos[0], nextPos[1], nextPos[2], 0.0, 0.0, 0.0, 0.2);
  ...
  function drawScene() {
    ...
    for(i = 0; i < (currentAtom.trajectories.length + 1); i++){
      ...
      var viewWorldMatrix = utils.multiplyMatrices(viewMatrix, orbitsWorldMatrix);
      var projectionMatrix = utils.multiplyMatrices(perspectiveMatrix, viewWorldMatrix);
      gl.uniformMatrix4fv(matrixLocation, gl.FALSE, utils.transposeMatrix(projectionMatrix));
      gl.uniformMatrix4fv(normalMatrixPositionHandle, gl.FALSE, utils.transposeMatrix(utils.invertMatrix(utils.transposeMatrix(orbitsWorldMatrix))));
      gl.uniform1i(textLocation, 0);
      gl.uniform3f(program.eyePosUniform, cx, cy, cz);
      ...
      gl.bindVertexArray(vaoOrbits);
      gl.drawElements(gl.TRIANGLES, orbitsModel.indices.length, gl.UNSIGNED_SHORT, 0 );
